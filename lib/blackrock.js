...
class Blackrock extends Elf.Alone {
  ...
  async break(baseId) {
    const id = SmartId.from('rock', baseId);
    if (!(await RockLogic.exist(this.cryo, id))) {
      return;
    }
...
/**
 * Throw this exception in order to abort a rock.
 */
class BlackrockAbortError extends Error {
  constructor(ex) {
    super(ex.message);
    this.name = 'BlackrockAbortError';
    this.code = ex.code;
    this.stack = ex.stack;
    this.exception = ex;
  }
...
module.exports = {Blackrock, BlackrockLogic, BlackrockAbortError};
